...
# Flow Helper
# ==========
def get_default_param() -> pd.Series:
...
def get_list_stock() -> pd.DataFrame:
	dbs: Session = next(get_dbs())
...
def get_list_index() -> pd.DataFrame:
	dbs: Session = next(get_dbs())
	qry = dbs.query(ListIndex.code)
	list_index:pd.DataFrame = pd.read_sql(sql=qry.statement, con=dbs.bind) # type: ignore
